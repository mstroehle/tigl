...
#ifndef _TIGLAISColormapped_HeaderFile
#define _TIGLAISColormapped_HeaderFile
...
DEFINE_STANDARD_HANDLE(TIGLAISColormapped,AIS_InteractiveObject)
...
class TIGLAISColormapped : public AIS_InteractiveObject
{

public:

  //! Constructs the Triangulation display object <br>
  Standard_EXPORT   TIGLAISColormapped(const TopoDS_Shape&);
...
//! Each 32-bit color is Alpha << 24 + Blue << 16 + Green << 8 + Red <br>
  ...
  Standard_EXPORT     void SetShape(const TopoDS_Shape& aTriangulation) ;
  //! Returns Poly_Triangulation . <br>
  Standard_EXPORT     TopoDS_Shape GetShape() const;

  DEFINE_STANDARD_RTTIEXT(TIGLAISColormapped,AIS_InteractiveObject)
